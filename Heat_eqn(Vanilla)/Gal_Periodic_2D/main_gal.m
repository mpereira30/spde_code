...
u0 = vec2mat(u0,(J(1)+1)); 
u0 = u0';
% converts the vector vec into a matrix with matcol columns, creating one
% row at a time. But we want all X's along rows (not columns) 

% Gaussian temp distribution
% u0 = fspecial('gaussian', [(J(1)+1) (J(2)+1)], 10);

tic
[t,ut] = pde_twod_Gal(u0, T, a, N, J); 
toc
